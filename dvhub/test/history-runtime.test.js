...
test('history runtime derives current-year market premium from available monthly market values until the official annual value exists', () => {
    ...
    store: {
      ...
      listPriceSlots() {
        return [
          ...
          { ts: '2027-03-10T10:00:00.000Z', priceCtKwh: -1, priceEurMwh: -10 }
        ];
      }
    },
    getPricingConfig: () => ({
  ...
  assert.equal(year.kpis.premiumEligibleExportKwh, 1.5);
...
test('history runtime exposes monthly and cross-month weekly market premium rates for aggregate rows', () => {
  ...
  assert.equal(week.kpis.periodMarketValueCtKwh, 5.17);
  assert.equal(week.kpis.marketPremiumEur, 0.05);
  assert.equal(week.kpis.marketPremiumCtKwh, 3.03);
  assert.equal(week.meta.marketPremium.displaySource, 'weighted_monthly');
  ...
  assert.equal(month.kpis.periodMarketValueCtKwh, 5.17);
  assert.equal(month.kpis.marketPremiumEur, 0.08);
  assert.equal(month.kpis.marketPremiumCtKwh, 3.03);
  assert.equal(month.meta.marketPremium.displaySource, 'official_annual');
  ...
  assert.equal(year.kpis.periodMarketValueCtKwh, 5.17);
...
test('history runtime uses monthly market value for month view when no annual value exists yet', () => {
  ...
  assert.equal(month.kpis.marketPremiumEur, 0.02);
...
test('history runtime sums yearly premium from monthly market values when configured globally', () => {
  ...
  assert.equal(year.rows[0].marketPremiumEur, 0.03);
  assert.equal(year.rows[1].marketPremiumEur, 0.09);
  assert.equal(year.kpis.periodMarketValueCtKwh, 4.79);
  assert.equal(year.kpis.marketPremiumEur, 0.12);
  assert.equal(year.kpis.marketPremiumCtKwh, 3.41);
  assert.equal(year.meta.marketPremium.source, 'configured_monthly');
...
test('history runtime derives annual premium eur from the displayed annual premium rate for all eligible export in running annual mode', () => {
  ...
  assert.equal(year.kpis.marketPremiumCtKwh, -3.82);
...
test('history runtime exposes pv full-load hours from configured plant capacity', () => {
  const runtime = createHistoryRuntime({
    ...
    getPricingConfig: () => ({
      ...
      pvPlants: [
        { kwp: 10, commissionedAt: '2021-04-15' },
        { kwp: 19.7, commissionedAt: '2023-09-01' }
      ]
    }),
    getCurrentDate: () => FIXED_CURRENT_DATE
  });
